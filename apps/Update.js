...
export class Update extends plugin {
  ...
  async runUpdate(isForce) {
    let command = 'git -C ./plugins/olympic-plugin/ pull --no-rebase'
    if (isForce) {
      command = `git -C ./plugins/olympic-plugin/ checkout . && ${command}`
      this.e.reply('正在执行强制更新操作，请稍等')
    } else {
      this.e.reply('正在执行更新操作，请稍等')
    }
    /** 获取上次提交的commitId，用于获取日志时判断新增的更新日志 */
    this.oldCommitId = await this.getcommitId('olympic-plugin')
    uping = true
    const ret = await this.execSync(command)
    uping = false

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：olympic-plugin`)
      this.gitErr(ret.error, ret.stdout)
      return false
    }

    /** 获取插件提交的最新时间 */
    const time = await this.getTime('olympic-plugin')

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`olympic-plugin已经是最新版本\n最后更新时间：${time}`)
    } else {
      await this.reply(`olympic-plugin\n最后更新时间：${time}`)
      this.isUp = true
      /** 获取olympic-plugin的更新日志 */
      const log = await this.getLog('olympic-plugin')
  ...
  async getLog(plugin = '') {
    ...
    let end = ''
      ...
      '更多详细信息，请前往github查看\nhttps://github.com/xiaomifan233/olympic-plugin/commits/main'

    log = await this.makeForwardMsg(`olympic-plugin更新日志，共${line}条`, log, end)
